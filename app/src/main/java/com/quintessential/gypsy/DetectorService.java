...
public class DetectorService extends Service {
    ...
    public static String FACEON=DetectorService.class.getName();
    public static String FACENOW="";
    public static String FACECOUNT="";
    public static String FACESMILE="";
    public static String FACEGENDER="";
    public void sendBroadcastMessage(String w, String x, String y, String z,String a){
        Intent intent=new Intent(FACEON);
        intent.putExtra(FACENOW,w+"@"+x+"@"+y+"@"+z+"@"+a);
        LocalBroadcastManager.getInstance(DetectorService.this).sendBroadcast(intent);
    }
    @Nullable
    @Override
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (!CameraHelper.checkPermission(this)) {
            Log.w(MainApplication.LOG_TAG, "app does not have camera permission, stopping service");
            stopSelf();
    ...
    private class DetectionHandler extends Handler {
        ...
        private DetectionHandler(Context context, HandlerThread detectionThread) {
            ...
            frameDetector = new FrameDetector(context,3,Detector.FaceDetectorMode.LARGE_FACES);
            ...
            frameDetector.setDetectAge(true);
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case START:
                    Log.d(MainApplication.LOG_TAG, "starting background processing of frames");
                    try {
                        frameDetector.start();
                        ...
                        Log.d(MainApplication.LOG_TAG, "couldn't open camera: " + e.getMessage());
                        return;
                    }
                    break;
                case STOP:
                    Log.d(MainApplication.LOG_TAG, "stopping background processing of frames");
                    cameraHelper.stop(); // stops previewing
                    cameraHelper.release();
                    frameDetector.stop();

                    Log.d(MainApplication.LOG_TAG, "quitting detection thread");
        ...
        private class CameraHelperListener implements CameraHelper.Listener {
            ...
            public void onFrameAvailable(byte[] frame, int width, int height, Frame.ROTATE rotation) {
                Log.d(MainApplication.LOG_TAG, "received frame");
         ...
         */
        private class DetectorListener implements FrameDetector.ImageListener, FrameDetector.FaceListener {
            ...
            public void onImageResults(List<Face> faces, Frame frame, float v) {
               ...
               if (faces.size() > 0) {
                    ...
                    Log.d(MainApplication.LOG_TAG, "valence score = " + Float.toString(face.emotions.getValence()));
                }

                if (faces.size() == 0) {
                    sendBroadcastMessage("N","0","0","-","-");

                } else {
                    Face face = faces.get(0);
                    sendBroadcastMessage("Y",String.valueOf(faces.size()),String.format("%.2f", face.expressions.getSmile()),String.valueOf(face.appearance.getGender()).toLowerCase(),String.valueOf(face.appearance.getAge()).toLowerCase());
                }
            }
        }
    }
